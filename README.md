...
Deploy a staging build to `summer-into-ai/usa-os-staging/`:

```powershell
node scripts/deploy-azure.mjs --dir usa-os-staging
```
...
acquired, and the ongoing instability pressure scales with blue CPU production
and green deployment throughput. Early instability is forgiving, but the
pressure multipliers rise every era. Later historical unlocks cost dramatically
more, turning the networked/present-day eras into a true long-game climb. Rapid
expansion has to be paced. Institutions counter instability generated by rapid growth
and build Civic Capacity, which lowers History install costs with a capped discount.
Uncontrolled instability causes crises and destroys some unspent progress.

Progress autosaves in `localStorage`, including capped offline production. Unlocking every
achievement starts the final bear-boss encounter. For staging/testing, type the
cheat code `usa250` to trigger that finale immediately.
